...
//Block Structure
struct loCOMMON_API CloBlock
{
	//Constructors
	CloBlock(unsigned int l=0, unsigned int r=0) : m_uil(l), m_uir(r) {}
	//Copy Constructor
	CloBlock(const CloBlock& roBlock) : m_uil(roBlock.m_uil), m_uir(roBlock.m_uir) {}
	CloBlock& operator^=(CloBlock& b) { m_uil ^= b.m_uil; m_uir ^= b.m_uir; return *this; }
	unsigned int m_uil, m_uir;
};

class loCOMMON_API CloBlowFish
{
public:
	enum { ECB=0, CBC=1, CFB=2 };

	//Constructor - Initialize the P and S boxes for a given Key
	CloBlowFish(unsigned char* ucKey, size_t n, const CloBlock& roChain = CloBlock(0UL,0UL));
...
private:
	...
	void Encrypt(CloBlock&);
	void Decrypt(CloBlock&);
...
//Function F
inline unsigned int CloBlowFish::F(unsigned int ui)
{
